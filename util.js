function getHeaderForLogging() {
  const timestampWidth = 60;
  const eventWidth = 10;
  const detailsWidth = 50;

  // Format the header using string padding
  return `Timestamp${" ".repeat(
    timestampWidth - "Timestamp".length
  )}Event${" ".repeat(eventWidth - "Event".length)}Details`;
}

const Events = {
  PRICE_CHECK: "CHECKING_PRICE",
  PRICE_CHANGE_UP: "PRICE_WENT_UP",
...
module.exports = { getHeaderForLogging, Events, getRandomUserAgent };
